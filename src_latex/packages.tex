...
% Don't panic on the following Unicode characters in listings, replace them instead
\lstset{literate={’}{{'}}1}
\lstset{literate={…}{{...}}1}
...
% Advanced bibliography with biblatex
\usepackage[
    backend=bibtex,
    %% use date like "Apr. 7, 2017" instead of American "04/07/2017", to avoid
    %% confusion over MDY vs DMY
    urldate=long,
...
% Allow grouped citations (if biblatex not allowed)
%
% e.g. \cite{citation01,citation02,}
%
% Note: Unlike biblatex, cite does not like trailing commas in the citation listings.
% A trailing comma will result in a null reference which becomes unknown.
% To fix it, delete the trailing comma
%\usepackage{cite}
...
% Must be included after amsmath (and glossaries, because glossaries brings in amsmath)
\usepackage{cleveref}
